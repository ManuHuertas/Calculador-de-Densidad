...
const PARTICLE_POOL = Array.from({ length: 500 }).map((_, i) => ({
  id: i,
  x: Math.random(),
  y: Math.random(),
  r: 1 + Math.random() * 1.5,
...
const Visualizer: React.FC<VisualizerProps> = ({ mass, volume, materialColor, liquidDensity = 1.0 }) => {
  ...
  const visibleParticles = useMemo(() => {
    const count = Math.min(Math.floor(mass / 2), 500);
    return PARTICLE_POOL.slice(0, count);
  }, [mass]);
      ...
      <svg className="w-full h-full" viewBox="0 0 800 600" preserveAspectRatio="xMidYMid meet">
        ...
        <g transform={`translate(${400 - size / 2}, ${yPos})`} style={{ transition: 'transform 0.2s ease-out' }}>
          <rect 
            ...
            height={size} 
            rx="8" 
            className={`${materialColor || 'fill-white'} stroke-[2px] ${density > liquidDensity ? 'stroke-rose-500' : 'stroke-emerald-400'}`} 
          />
